...
function matchesInternalUrl(url) {
...
export function evaluate(req, custom) {
    ...
    if (name === "exec" || name === "bash" || name === "shell" || name === "process") {
        ...
        if (match)
            return { decision: "deny", reason: `dangerous_command: ${match}` };
        if (custom?.denyCommands) {
            for (const pattern of custom.denyCommands) {
                if (safeRegexTest(pattern, command))
                    return { decision: "deny", reason: `custom_deny_command: ${pattern}` };
            }
        }
    }
    if (name === "write" || name === "edit" || name === "apply_patch") {
        ...
        if (sensitive)
            return { decision: "deny", reason: `sensitive_path: ${sensitive}` };
        if (custom?.denyPaths && path) {
            const normalized = path.replace(/\\/g, "/");
            for (const pattern of custom.denyPaths) {
                if (normalized.includes(pattern))
                    return { decision: "deny", reason: `custom_deny_path: ${pattern}` };
            }
        }
    ...
    if (name === "browser" || name === "navigate" || name === "web_fetch") {
        ...
        if (url && matchesInternalUrl(url))
            return { decision: "ask", reason: `internal_url: ${url}` };
    }
    return { decision: "allow", reason: "no_matching_rule" };
}
