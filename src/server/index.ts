...
import type { PolicyRequest, CustomRules, DaemonStats } from "../rules/types.js";

// ── State ──

let customRules: CustomRules | undefined;
...
export function startServer(opts?: ServerOptions): ReturnType<typeof createServer> {
  ...
  server.listen(port, () => {
    ...
    if (customRules) console.error(`[clawwall] loaded custom rules from ${rulesFile}`);
  });
...
async function handleRequest(
  ...
  broadcast: WsBroadcaster["broadcast"],
) {
  // POST /policy/check
  if (req.method === "POST" && req.url === "/policy/check") {
    let body: PolicyRequest;
    try {
      body = JSON.parse(await readBody(req)) as PolicyRequest;
    } catch {
    ...
    if (decision.decision === "ask") {
      stats.ask++;
      const finalDecision = await approvals.request(
        body.tool.name,
        body.tool.args,
        decision.reason,
        broadcast,
    ...
    if (decision.decision === "allow") stats.allow++;
    else if (decision.decision === "deny") stats.deny++;
