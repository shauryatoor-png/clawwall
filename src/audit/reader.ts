...
export function readAuditLog(dir: string, maxLines = 100): AuditEntry[] {
  try {
    ...
    return lines.slice(-maxLines).map((l) => JSON.parse(l) as AuditEntry);
  } catch {
    return [];
  }
}
    ...
    try {
      ...
      if (content.length > lastSize) {
        ...
        for (const line of lines) {
          try {
            onEntry(JSON.parse(line) as AuditEntry);
